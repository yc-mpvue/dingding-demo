<template>
  <div class="test">
    ...
    <canvas id="area"
      ...
      :width="width"
      :height="height" />
    <!-- <f2-cavas @drawChart='drawChart' /> -->
  </div>
</template>

<script>
import F2 from "@antv/my-f2";
import f2Cavas from '@/components/f2Cavas';
let canvas
export default {
  data () {
    return {
  ...
  onReady () {
    my
      ...
      .exec(res => {
        ...
        this.canvas = canvas;//保留cavas对象,在下面点击事件里要用
        this.drawChart1(canvas, res[0].width, res[0].height);
  ...
  methods: {
    ...
    drawChart1 (canvas, width, height) {
    ...
    drawChart2 (canvas, width, height) {
